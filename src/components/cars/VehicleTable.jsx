      ...
      <tbody className="divide-y divide-slate-100">
        {cars.map((car) => {
          ...
          return <tr key={car.id} className={`${selectedIds.has(car.id) ? 'bg-teal-50/60' : 'hover:bg-slate-50/80'} transition`}>
            ...
            <td className="px-3 py-3"><button type="button" onClick={() => onView(car)} className="max-w-52 text-start text-sm font-black text-slate-800 hover:text-teal-700">{car.yearMakeModel}</button><p className="mt-1 text-[10px] text-slate-400">{car.createdAt ? new Date(car.createdAt).toLocaleDateString() : '—'}</p></td>
