...
export default function SubscriptionPage() {
  ...
  const [workspace, setWorkspace] = useState({ plans: [], subscription: null, invoices: [], memberCount: 0, activeVehicleCount: 0, storageMb: 0, usage: null, entitlements: [], entitlement: null, isPlatformAdmin: false, pendingPayments: [] });
  ...
  const showTrialWarning = current?.status === 'trialing' && remaining !== null && remaining <= 5;
  const showDueWarning = openInvoice && new Date(`${openInvoice.due_date}T23:59:59`) <= new Date();
  ...
  return <div className="space-y-5">
    ...
    {(showTrialWarning || showDueWarning) && <div className={`rounded-xl border px-4 py-3 font-bold text-sm flex items-start gap-2 ${showDueWarning ? 'bg-red-50 border-red-200 text-red-800' : 'bg-amber-50 border-amber-200 text-amber-800'}`}><HiOutlineClock className="w-5 h-5 shrink-0" />{showDueWarning ? labels.dueWarning : labels.trialWarning}</div>}

    <div className="grid xl:grid-cols-[1.1fr_1fr] gap-4">
      <section className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 text-white p-6 shadow-lg"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-slate-400">{labels.currentPlan}</p><h2 className="text-3xl font-black mt-1">{lang === 'ar' ? plan?.name_ar : plan?.name_en}</h2><span className={`inline-block mt-3 px-3 py-1 rounded-full text-xs font-bold ${statusClass(current?.status)}`}>{statusLabel(current?.status)}</span></div><HiOutlineShieldCheck className="w-12 h-12 text-teal-300" /></div><div className="grid sm:grid-cols-2 gap-3 mt-7"><div className="rounded-xl bg-white/5 border border-white/10 p-4"><p className="text-xs text-slate-400">{current?.status === 'trialing' ? labels.trialEnds : labels.periodEnds}</p><strong className="block mt-1">{date(currentEnd, lang)}</strong></div><div className="rounded-xl bg-white/5 border border-white/10 p-4"><p className="text-xs text-slate-400">{labels.daysRemaining}</p><strong className="block text-2xl mt-1" dir="ltr">{remaining ?? '—'}</strong></div></div>{workspace.entitlement && <div className={`mt-3 rounded-xl border px-4 py-3 flex items-center gap-2 text-sm font-bold ${workspace.entitlement.can_create_vehicle ? 'bg-emerald-400/10 border-emerald-300/30 text-emerald-200' : 'bg-red-400/10 border-red-300/30 text-red-200'}`}>{workspace.entitlement.can_create_vehicle ? <HiOutlineCheck className="w-5 h-5" /> : <HiOutlineX className="w-5 h-5" />}{workspace.entitlement.can_create_vehicle ? labels.entitlementAllowed : labels.entitlementBlocked}</div>}</section>
      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5"><h2 className="font-bold text-xl text-slate-800 mb-4">{labels.usage}</h2><div className="space-y-3"><UsageBar icon={HiOutlineUsers} label={labels.users} used={workspace.memberCount} limit={plan?.max_users} unlimitedLabel={labels.unlimited} /><UsageBar icon={HiOutlineTruck} label={labels.activeVehicles} used={workspace.activeVehicleCount} limit={plan?.max_active_vehicles} unlimitedLabel={labels.unlimited} /><UsageBar icon={HiOutlineDocumentText} label={labels.storage} used={workspace.storageMb} limit={plan?.max_storage_mb} unlimitedLabel={labels.unlimited} /></div><div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-xl bg-slate-50 p-3 text-center"><strong className="block text-xl text-slate-900" dir="ltr">{workspace.usage?.active_users ?? 0}</strong><small className="text-slate-500">{labels.activeUsers}</small></div><div className="rounded-xl bg-slate-50 p-3 text-center"><strong className="block text-xl text-slate-900" dir="ltr">{workspace.usage?.session_count ?? 0}</strong><small className="text-slate-500">{labels.sessions}</small></div><div className="rounded-xl bg-slate-50 p-3 text-center"><strong className="block text-xl text-slate-900" dir="ltr">{workspace.usage?.action_count ?? 0}</strong><small className="text-slate-500">{labels.actions}</small></div></div></section>
    </div>
    ...
    <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden"><div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2"><HiOutlineDocumentText className="text-teal-600" /><h2 className="font-bold text-xl text-slate-800">{labels.invoices}</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[900px]"><thead className="bg-slate-50 text-xs text-slate-500"><tr>{[labels.invoiceNumber, labels.plan, labels.cycle, labels.issueDate, labels.dueDate, labels.total, labels.invoiceStatus, labels.action].map((item) => <th key={item} className="px-4 py-3 text-start">{item}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{workspace.invoices.length === 0 ? <tr><td colSpan="8" className="py-14 text-center text-slate-400">{labels.noInvoices}</td></tr> : workspace.invoices.map((invoice) => { const pending = invoice.subscription_payments?.some((payment) => payment.status === 'pending'); const confirmed = invoice.subscription_payments?.find((payment) => payment.status === 'confirmed'); return <tr key={invoice.id}><td className="px-4 py-3 font-mono"><Link to={`/subscription/invoices/${invoice.id}/print`} className="text-teal-700 hover:underline">{invoice.invoice_number}</Link></td><td className="px-4 py-3 font-bold">{lang === 'ar' ? invoice.plans?.name_ar : invoice.plans?.name_en}</td><td className="px-4 py-3">{invoice.billing_cycle === 'monthly' ? labels.monthly : labels.annual}</td><td className="px-4 py-3">{date(invoice.issue_date, lang)}</td><td className="px-4 py-3">{date(invoice.due_date, lang)}</td><td className="px-4 py-3 font-bold" dir="ltr">{money(invoice.total, lang)}</td><td className="px-4 py-3"><span className={`px-2.5 py-1 rounded-full text-xs font-bold ${invoice.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : invoice.status === 'issued' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'}`}>{labels[invoice.status] || invoice.status}</span></td><td className="px-4 py-3"><div className="flex flex-wrap items-center gap-3"><Link to={`/subscription/invoices/${invoice.id}/print`} className="text-sm font-bold text-slate-600 flex items-center gap-1"><HiOutlinePrinter />{labels.print}</Link>{confirmed && <Link to={`/subscription/payments/${confirmed.id}/receipt`} className="text-sm font-bold text-emerald-700">{labels.receipt}</Link>}{invoice.status === 'issued' && (pending ? <span className="text-xs font-bold text-amber-700"><HiOutlineClock className="inline" /> {labels.paymentPending}</span> : role === 'owner' && <button onClick={() => setPaymentInvoice(invoice)} className="text-sm font-bold text-teal-700">{labels.pay}</button>)}</div></td></tr>; })}</tbody></table></div></section>
