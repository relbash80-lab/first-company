...
export default function LoginPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (mode === 'reset') {
        if (!resetToken) throw new Error(ar ? 'رابط إعادة التعيين غير صالح' : 'Invalid reset link');
        if (password !== passwordConfirmation) throw new Error(ar ? 'كلمتا المرور غير متطابقتين' : 'Passwords do not match');
        await completePasswordReset(resetToken, password);
        window.history.replaceState({}, '', appPath('login'));
        setPassword('');
        setPasswordConfirmation('');
        setMode('login');
        toast.success(ar ? 'تم تعيين كلمة المرور. يمكنك تسجيل الدخول الآن.' : 'Password set. You can sign in now.');
      } else if (mode === 'signup') {
  ...
  const appPath = (path) => `${import.meta.env.BASE_URL}${path}`.replace(/\/+/g, '/');
      ...
      <section className="hidden lg:flex relative overflow-hidden p-14 text-white flex-col justify-between bg-gradient-to-br from-teal-700 via-teal-900 to-slate-950">
        ...
        <div className="relative max-w-xl">
          ...
          <div className="space-y-4 text-slate-200">
            {[ar ? 'عزل كامل لبيانات كل مشترك' : 'Tenant data isolation', ar ? 'رصيد مشتريات وشحن محسوب آليًا' : 'Automatic purchase and shipping balances', ar ? 'تحديثات ذكية وسجل تدقيق' : 'Smart updates and audit history'].map((item) => <div key={item} className="flex items-center gap-3"><HiOutlineCheckCircle className="text-teal-300 w-6 h-6" />{item}</div>)}
          </div>
        </div>
        <p className="relative text-xs text-slate-400">Powered by Neon · Secure multi-tenant SaaS</p>
      </section>

      <section className="bg-slate-50 flex items-center justify-center p-5 sm:p-10">
        <div className="w-full max-w-md">
          <div className="flex justify-between items-center mb-8"><div className="lg:hidden font-black text-slate-900">First Company</div><button onClick={toggleLanguage} className="min-h-11 px-2 text-sm font-bold text-teal-700">{ar ? 'English' : 'العربية'}</button></div>
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/60 p-7 sm:p-9">
            {mode !== 'reset' && <div className="grid grid-cols-2 bg-slate-100 p-1 rounded-xl mb-7">
              <button onClick={() => setMode('login')} className={`min-h-11 rounded-lg py-2.5 text-sm font-bold ${mode === 'login' ? 'bg-white shadow text-teal-700' : 'text-slate-500'}`}>{ar ? 'تسجيل الدخول' : 'Sign in'}</button>
              <button onClick={() => setMode('signup')} className={`min-h-11 rounded-lg py-2.5 text-sm font-bold ${mode === 'signup' ? 'bg-white shadow text-teal-700' : 'text-slate-500'}`}>{ar ? 'إنشاء حساب' : 'Create account'}</button>
            </div>}
            <h2 className="text-2xl font-black text-slate-900">{mode === 'reset' ? (ar ? 'عيّن كلمة مرور جديدة' : 'Set a new password') : mode === 'login' ? (ar ? 'مرحبًا بعودتك' : 'Welcome back') : (ar ? 'ابدأ تجربتك' : 'Start your trial')}</h2>
            <p className="text-slate-500 mt-2 mb-7">{mode === 'reset' ? (ar ? 'اختر كلمة مرور قوية لحسابك في Neon.' : 'Choose a strong password for your Neon account.') : mode === 'login' ? (ar ? 'ادخل إلى مساحة شركتك الآمنة.' : 'Enter your secure company workspace.') : (ar ? 'أنشئ حسابك ثم مساحة شركتك.' : 'Create your account, then your workspace.')}</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {mode === 'reset' && <label className="block"><span className="text-sm font-semibold text-slate-700">{ar ? 'تأكيد كلمة المرور' : 'Confirm password'}</span><input type="password" value={passwordConfirmation} onChange={(e) => setPasswordConfirmation(e.target.value)} required minLength="8" className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:ring-2 focus:ring-teal-500" dir="ltr" placeholder="••••••••" /></label>}
              <button disabled={loading} className="w-full bg-teal-600 hover:bg-teal-700 text-white rounded-xl py-3.5 font-black flex items-center justify-center gap-2 disabled:opacity-50"><HiOutlineArrowRightOnRectangle />{loading ? (ar ? 'جارٍ التنفيذ...' : 'Please wait...') : mode === 'reset' ? (ar ? 'حفظ كلمة المرور' : 'Save password') : mode === 'login' ? (ar ? 'دخول' : 'Sign in') : (ar ? 'إنشاء الحساب' : 'Create account')}</button>
              {mode === 'login' && <button type="button" onClick={handleReset} className="min-h-11 w-full py-2 text-sm font-semibold text-teal-700">{ar ? 'نسيت كلمة المرور؟' : 'Forgot password?'}</button>}
            </form>
          </div>
        </div>
      </section>
