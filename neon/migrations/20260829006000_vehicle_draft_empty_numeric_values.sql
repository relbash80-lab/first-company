...
declare
  ...
  entitlement record;
  item jsonb;
  purchase_paid numeric := nullif(trim(p_payload->>'purchasePaid'), '')::numeric;
  shipping_paid numeric := nullif(trim(p_payload->>'shippingPaid'), '')::numeric;
begin
  ...
  if p_vehicle_id is null then
    if coalesce(purchase_paid, 0) > 0 then
      insert into public.payments (organization_id, vehicle_id, type, amount, reference, notes)
      values (p_organization_id, result_vehicle_id, 'purchase', purchase_paid, 'opening-entry', 'Initial amount entered while creating the vehicle');
    end if;
    if coalesce(shipping_paid, 0) > 0 then
      insert into public.payments (organization_id, vehicle_id, type, amount, reference, notes)
      values (p_organization_id, result_vehicle_id, 'shipping', shipping_paid, 'opening-entry', 'Initial amount entered while creating the vehicle');
    end if;
  end if;
