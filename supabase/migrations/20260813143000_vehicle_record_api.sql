...
grant select on public.platform_admins, public.billing_events to authenticated;
...
begin
  ...
  if client_name is not null then
    select id into result_client_id
    from public.clients
    where organization_id = p_organization_id
      and lower(name) = lower(client_name)
    order by created_at
    limit 1;

    if result_client_id is null then
      insert into public.clients (organization_id, name)
      values (p_organization_id, client_name)
  ...
  if p_vehicle_id is null then
    insert into public.vehicles (
      ...
      p_organization_id,
      result_client_id,
      upper(trim(p_payload->>'vin')),
      nullif(p_payload->>'year', '')::smallint,
      nullif(trim(p_payload->>'make'), ''),
      nullif(trim(p_payload->>'model'), ''),
      nullif(trim(p_payload->>'trim'), ''),
      coalesce(nullif(p_payload->>'auction', ''), 'Other'),
      nullif(trim(p_payload->>'lotStock'), ''),
      nullif(trim(p_payload->>'buyingLocation'), ''),
      nullif(p_payload->>'buyingDate', '')::date,
      nullif(p_payload->>'wireDate', '')::date,
      coalesce(nullif(p_payload->>'status', '')::public.vehicle_status, 'purchased'),
    ...
    update public.vehicles set
      ...
      make = nullif(trim(p_payload->>'make'), ''),
      model = nullif(trim(p_payload->>'model'), ''),
      trim = nullif(trim(p_payload->>'trim'), ''),
      auction = coalesce(nullif(p_payload->>'auction', ''), 'Other'),
      lot_stock = nullif(trim(p_payload->>'lotStock'), ''),
      buying_location = nullif(trim(p_payload->>'buyingLocation'), ''),
      buying_date = nullif(p_payload->>'buyingDate', '')::date,
      purchase_wire_date = nullif(p_payload->>'wireDate', '')::date,
      status = coalesce(nullif(p_payload->>'status', '')::public.vehicle_status, 'purchased'),
      notes = nullif(trim(p_payload->>'notes'), ''),
      updated_at = now()
    where id = p_vehicle_id and organization_id = p_organization_id
    returning id into result_vehicle_id;

    if result_vehicle_id is null then
  ...
  for item in select value from jsonb_array_elements(coalesce(p_payload->'charges', '[]'::jsonb)) loop
    if coalesce((item->>'amount')::numeric, 0) > 0 then
      insert into public.charges (
        organization_id, vehicle_id, category, description, amount, source
      ) values (
        p_organization_id,
        result_vehicle_id,
        (item->>'category')::public.charge_category,
  ...
  if container_number is not null then
    insert into public.containers (
      ...
      p_organization_id,
      upper(container_number),
      nullif(trim(p_payload->>'shippingLine'), ''),
      nullif(trim(p_payload->>'shippingPort'), ''),
      nullif(trim(p_payload->>'destination'), ''),
      nullif(p_payload->>'transitArrivalDate', '')::date,
      nullif(p_payload->>'shippingWireDate', '')::date
